...
fn main() {
    ...
    generate_bpe(&data_dir, &out_dir, "claude", "CLAUDE");
    generate_bpe(&data_dir, &out_dir, "deepseek", "DEEPSEEK");
    generate_bpe(&data_dir, &out_dir, "qwen", "QWEN");
    generate_bpe(&data_dir, &out_dir, "glm", "GLM");

    println!("cargo::rerun-if-changed=data/models.json");
    println!("cargo::rerun-if-changed=data/claude.json");
    println!("cargo::rerun-if-changed=data/deepseek.json");
    println!("cargo::rerun-if-changed=data/qwen.json");
    println!("cargo::rerun-if-changed=data/glm.json");
}
...
fn generate_bpe(data_dir: &Path, out_dir: &str, stem: &str, prefix: &str) {
    let json = fs::read_to_string(data_dir.join(format!("{stem}.json"))).unwrap();
    let data: BpeJsonData = serde_json::from_str(&json).unwrap();
    ...
    fs::write(Path::new(out_dir).join(format!("{stem}_bpe.bin")), &bin).unwrap();

    // Generate Rust source with the encoding's metadata
    let mut out = String::with_capacity(1024);
    out.push_str("// Generated by build.rs — do not edit\n\n");
    out.push_str(&format!(
        "pub(crate) const {prefix}_PAT_STR: &str = {:?};\n\n",
        data.pat_str
    ));
    out.push_str(&format!(
        "pub(crate) const {prefix}_TOKEN_COUNT: u32 = {token_count};\n\n"
    ));
    out.push_str(&format!(
        "pub(crate) const {prefix}_SPECIAL_TOKENS: &[(&str, u32)] = &[\n"
    ));
    ...
    out.push_str(&format!(
        "pub(crate) const {prefix}_BPE_DATA: &[u8] = include_bytes!(concat!(env!(\"OUT_DIR\"), \"/{stem}_bpe.bin\"));\n",
    ));

    fs::write(Path::new(out_dir).join(format!("{stem}_generated.rs")), out).unwrap();
}
