//! Support for capturing other fields from [alloy-serde](https://docs.rs/alloy-serde/latest/alloy_serde/)
...
mod tests {
    ...
    fn test_correct_other() {
        ...
        assert_eq!(
            with_other.other,
            OtherFields::new(BTreeMap::from_iter([(
                "b".to_string(),
                serde_json::json!(2)
            )]))
        );
    ...
    fn test_with_other_fields_serialization() {
        ...
        let with_other = WithOtherFields {
            inner,
            other: OtherFields::new(other.clone()),
        };
